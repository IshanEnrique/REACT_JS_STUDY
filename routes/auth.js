...
router.post(
  ...
  [
    ...
    body("mobile","Enter a valid mobile").escape()
    .exists({checkFalsy: true})
    .isLength({min: 10, max:10})
    .matches(/^([0|\+[0-9]{1,5})?([7-9][0-9]{9})$/),
    body("password","Password should contain minimum eight characters, at least one letter, one number and one special character.").escape()
    .exists({checkFalsy: true})
    .isLength({min: 8})
